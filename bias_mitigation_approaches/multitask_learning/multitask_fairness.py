# bias_mitigation_approaches/multitask_learning/multitask_fairness.py
...
from model.multitask_model import create_multitask_model
from model.fine_tuning import progressive_fine_tuning
from utils.data_loader import load_combined_rafdb_dataset, prepare_multitask_data, demographic_stratified_split
from utils.metrics import evaluate_multitask_model
...
def main():
    ...
    print(f"Output model: {MULTITASK_MODEL_OUTPUT_PATH}")
    ...
    train_data = prepare_multitask_data(X_train, e_train, g_train, a_train)
    val_data = prepare_multitask_data(X_val, e_val, g_val, a_val)
    test_data = prepare_multitask_data(test_images, test_emotions, test_gender, test_age)
    ...
    model, feature_extractor = create_multitask_model()
    ...
    evaluation_results = evaluate_multitask_model(model, test_data[0], test_data[1])
    ...
    print(f"Model saved to {MULTITASK_MODEL_OUTPUT_PATH}")
