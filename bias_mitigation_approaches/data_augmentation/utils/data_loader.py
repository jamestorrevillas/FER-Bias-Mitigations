# bias_mitigation_approaches/data_augmentation/utils/data_loader.py
...
def load_augmented_rafdb_dataset():
    ...
    if not os.path.exists(config.RAFDB_AUGMENTED_IMAGES_PATH):
        raise FileNotFoundError(f"Augmented images file not found at {config.RAFDB_AUGMENTED_IMAGES_PATH}")
    
    if not os.path.exists(config.RAFDB_AUGMENTED_LABELS_PATH):
        raise FileNotFoundError(f"Augmented labels file not found at {config.RAFDB_AUGMENTED_LABELS_PATH}")
    
    if not os.path.exists(config.RAFDB_AUGMENTED_DEMOGRAPHICS_PATH):
        raise FileNotFoundError(f"Augmented demographics file not found at {config.RAFDB_AUGMENTED_DEMOGRAPHICS_PATH}")
    
    try:
        # Load augmented images
        augmented_images = np.load(config.RAFDB_AUGMENTED_IMAGES_PATH)
        print(f"Loaded {len(augmented_images)} augmented images with shape {augmented_images.shape}")
        
        # Load augmented labels
        augmented_labels = np.load(config.RAFDB_AUGMENTED_LABELS_PATH)
        print(f"Loaded {len(augmented_labels)} augmented labels")
        
        # Load demographic info
        with open(config.RAFDB_AUGMENTED_DEMOGRAPHICS_PATH, 'rb') as f:
            augmented_demographic_info = pickle.load(f)
        print(f"Loaded demographic information for {len(augmented_demographic_info)} augmented samples")
        
        return augmented_images, augmented_labels, augmented_demographic_info
